...
import com.leyou.common.parameter.pojo.BrandQueryByPageParameter;
...
public class BrandController {
     ...
     */
    @PostMapping
    @ApiOperation(value = "根据分类Ids保存相应的品牌接口",notes = "保存分类Ids下的品牌信息")
    @ApiImplicitParam(name = "categories",required = true,value = "地址对象")
    @ApiResponses({
            @ApiResponse(code = 201, message = "品牌保存成功")
     ...
     */
    @GetMapping("cid/{cid}")
    @ApiOperation(value = "根据分类id查询品牌列表，返回品牌列表信息",notes = "根据分类id查询品牌列表")
    @ApiImplicitParam(name = "cid",required = true,value = "分类id")
    @ApiResponses({
            @ApiResponse(code = 200, message = "品牌列表"),
